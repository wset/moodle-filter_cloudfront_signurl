 ...
 */
 ...
 // Cloudfront signing functions from Amazon http://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/CreateURL_PHP.html
...
function create_stream_name($stream, $policy, $signature, $expires) {
    $result = $stream;
...
function get_canned_policy_stream_name($video_path) {
    ...
    $stream_name = create_stream_name($video_path, null, $encoded_signature, $expires);
    // url-encode the query string characters to work around a flash player bug
    return $stream_name;
}

function get_custom_policy_stream_name($video_path, $policy) {
    // the policy contains characters that cannot be part of a URL, so we base64 encode it
    $encoded_policy = url_safe_base64_encode($policy);
    // sign the original policy, not the encoded version
    $signature = rsa_sha1_sign($policy);
    // make the signature safe to be included in a url
    $encoded_signature = url_safe_base64_encode($signature);

    // combine the above into a stream name
    $stream_name = create_stream_name($video_path, $encoded_policy, $encoded_signature, null);
    // url-encode the query string characters to work around a flash player bug
    return $stream_name;
}
